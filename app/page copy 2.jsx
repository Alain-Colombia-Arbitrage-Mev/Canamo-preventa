...
export default function Home() {
  ...
  const handleAmountUSDChange = event => {
    ...
    setAmountRaiX(amount / 0.75);
  };

  const handleAmountRaiXChange = event => {
    const amount = Math.abs(Number(event.target.value));
    setAmountRaiX(amount);
    setAmountUSD(Number(amount * 0.75));
  };
  ...
  const {
    ...
    args: [
        Number(selectedToken?.id || 0),
        parseUnits(amountUSD.toString(), selectedToken?.decimals || 18)
    ],
    // Agregamos onError y onSuccess para mejor manejo
    onError: (error) => {
...
const handleTokenSelect = (token) => {
  console.log("Token seleccionado:", token); // Para debug
  ...
  const actionHandler = useMemo(() => {
    ...
    return (
        <button
            onClick={() => {
                if (!buyHandler) {
                    console.error('Buy handler not available');
                    return;
                }
                
                try {
                    console.log('Attempting to buy with:', {
            ...
            disabled={!buyHandler || isBuyLoading || amountUSD <= 0}
            className={`w-40 h-30 ${
                !buyHandler || isBuyLoading || amountUSD <= 0
...
, [isConnected, allowance, amountUSD, isApproveStarted, isApproveLoading, buyHandler,isBuyLoading, selectedToken,isBuyStarted]);
